...
const ROOT   = '/';
const URL    = ROOT + 'test/core/cases/';
const FOLDER = './test/core/cases/';
  ...
  test.describe(name, () => {
    ...
    if (!['case12'].includes(code)) {
      test('compare image', async ({page}) => {
        const component = page.locator('#case my-component');
    ...
    if (['case12'].includes(code)) {
      test('compare content after load', async ({page}) => {
        ...
        await expect(content).toContainText('hello Grapper')
      });
    }
